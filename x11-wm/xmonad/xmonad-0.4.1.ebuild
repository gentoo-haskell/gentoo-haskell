...
inherit eutils haskell-cabal savedconfig versionator

CONT_V=$(get_version_component_range 1-2)

DESCRIPTION="A lightweight X11 window manager"
HOMEPAGE="http://www.xmonad.org/"
SRC_URI="http://hackage.haskell.org/packages/archive/${PN}/${PV}/${P}.tar.gz
	extensions? ( http://www.xmonad.org/XMonadContrib-${CONT_V}.tar.gz )"
...
src_unpack() {
	...
	if use extensions ; then
		...
		sh XMonadContrib/scripts/generate-configs --main "${S}" \
			--contrib "${S}/XMonadContrib" || die "generate-configs failed"
		mv -f XMonadContrib/xmonad.cabal "${S}"
	fi
