...
# NOTE: when bumping using hackport, it may fail on the .cabal file.
# Download glirc's .cabal file and remove the trailing comma from the
# last entries in setup-depends and build-depends.
...
RDEPEND=">=dev-haskell/async-2.2:=[profile?] <dev-haskell/async-2.3:=[profile?]
	...
	>=dev-haskell/hookup-0.2.3:=[profile?] <dev-haskell/hookup-0.4:=[profile?]
	...
	>=dev-haskell/vty-5.23.1:=[profile?] <dev-haskell/vty-5.27:=[profile?]
	>=dev-lang/ghc-8.4.3:=
"
DEPEND="${RDEPEND}
...
src_prepare() {
	...
	cabal_chdeps \
		'regex-tdfa           >=1.3    && <1.4' 'regex-tdfa           >=1.3.1  && <1.4'
}
