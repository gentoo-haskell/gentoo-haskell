...
RDEPEND=">=dev-haskell/aeson-1.4.1.0:=[profile?] <dev-haskell/aeson-1.5:=[profile?]
	...
	>=dev-haskell/network-2.8:=[profile?] <dev-haskell/network-2.9:=[profile?]
	...
	>=dev-haskell/servant-0.15:=[profile?] <dev-haskell/servant-0.15.1:=[profile?]
	...
	>=dev-haskell/warp-3.2.25:=[profile?] <dev-haskell/warp-3.3:=[profile?]
...
src_install() {
	cabal_src_install
	# `greet` is an example program that is automatically built. It does not need
	# to be installed, but there is the option to do so via the `examples` USE flag.
	if ! use examples; then
		rm -v "$D/usr/bin/greet"
		rmdir -v "$D/usr/bin"
	fi
}
