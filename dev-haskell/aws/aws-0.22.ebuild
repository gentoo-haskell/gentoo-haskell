...
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE="examples" # networkbsd flag must be enabled when we migrate ::haskell to network-3
...
"
# networkbsd? ( >=dev-haskell/network-3:=[profile?] <dev-haskell/network-4:=[profile?]
# 		>=dev-haskell/network-bsd-2.8:=[profile?] <dev-haskell/network-bsd-2.9:=[profile?] )
# !networkbsd? ( >=dev-haskell/network-2:=[profile?] <dev-haskell/network-3:=[profile?] )
DEPEND="${RDEPEND}
