# Copyright 1999-2018 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=6

# Starting from 0.2.2, apply patches from upstream allowing it to
# build with ghc 8.6
MY_PV="0.2.2"
MY_P="${PN}-${MY_PV}"

# ebuild generated by hackport 0.5.5.9999

CABAL_FEATURES="lib profile haddock hoogle hscolour"
inherit haskell-cabal

DESCRIPTION="A sensible set of defaults for writing custom Preludes"
HOMEPAGE="https://github.com/sdiehl/protolude"
SRC_URI="mirror://hackage/packages/archive/${PN}/${MY_PV}/${MY_P}.tar.gz"

LICENSE="MIT"
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE=""

RDEPEND=">=dev-haskell/async-2.0:=[profile?] <dev-haskell/async-2.3:=[profile?]
	>=dev-haskell/hashable-1.2:=[profile?] <dev-haskell/hashable-1.3:=[profile?]
	>=dev-haskell/mtl-2.1:=[profile?] <dev-haskell/mtl-2.3:=[profile?]
	>=dev-haskell/mtl-compat-0.2:=[profile?] <dev-haskell/mtl-compat-0.3:=[profile?]
	>=dev-haskell/stm-2.4:=[profile?]
	>=dev-haskell/text-1.2:=[profile?] <dev-haskell/text-1.3:=[profile?]
	>=dev-haskell/transformers-compat-0.4:=[profile?] <dev-haskell/transformers-compat-0.7:=[profile?]
	>=dev-lang/ghc-7.8.2:=
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-1.18.1.3
"

PATCHES=("${FILESDIR}"/${PN}-0.2.2-ghc-8.6-p1.patch
		 "${FILESDIR}"/${PN}-0.2.2-ghc-8.6-p2.patch
		 "${FILESDIR}"/${PN}-0.2.2-ghc-8.6-p3.patch)

S="${WORKDIR}/${MY_P}"
