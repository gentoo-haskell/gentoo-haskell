# Copyright 1999-2015 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: $

EAPI=5

# ebuild generated by hackport 0.4.4

CABAL_FEATURES="bin lib profile haddock hoogle hscolour test-suite"
inherit haskell-cabal

DESCRIPTION="A Haskell backend kernel for the IPython project"
HOMEPAGE="http://gibiansky.github.io/IHaskell/"
SRC_URI="mirror://hackage/packages/archive/${PN}/${PV}/${P}.tar.gz"

LICENSE="MIT"
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE=""

RDEPEND=">=dev-haskell/aeson-0.6:=[profile?] <dev-haskell/aeson-0.9:=[profile?]
	>=dev-haskell/base64-bytestring-1.0:=[profile?]
	>=dev-haskell/cereal-0.3:=[profile?]
	>=dev-haskell/classy-prelude-0.9.5:=[profile?] <dev-haskell/classy-prelude-0.11:=[profile?]
	>=dev-haskell/cmdargs-0.10:=[profile?]
	>=dev-haskell/ghc-parser-0.1.4:=[profile?]
	>=dev-haskell/ghc-paths-0.1:=[profile?] <dev-haskell/ghc-paths-0.2:=[profile?]
	dev-haskell/haskeline:=[profile?]
	>=dev-haskell/haskell-src-exts-1.16:=[profile?] <dev-haskell/haskell-src-exts-1.17:=[profile?]
	>=dev-haskell/here-1.2:=[profile?] <dev-haskell/here-1.3:=[profile?]
	>=dev-haskell/hlint-1.9:=[profile?] <dev-haskell/hlint-2.0:=[profile?]
	dev-haskell/hspec:=[profile?]
	>=dev-haskell/http-client-0.4:=[profile?] <dev-haskell/http-client-0.5:=[profile?]
	>=dev-haskell/http-client-tls-0.2:=[profile?] <dev-haskell/http-client-tls-0.3:=[profile?]
	dev-haskell/hunit:=[profile?]
	>=dev-haskell/ipython-kernel-0.3:=[profile?]
	>=dev-haskell/missingh-1.2:=[profile?]
	>=dev-haskell/mono-traversable-0.6:=[profile?] <dev-haskell/mono-traversable-0.7:=[profile?]
	>=dev-haskell/mtl-2.1:=[profile?]
	dev-haskell/parsec:=[profile?]
	>=dev-haskell/random-1.0:=[profile?]
	>=dev-haskell/shelly-1.5:=[profile?] <dev-haskell/shelly-1.6:=[profile?]
	>=dev-haskell/split-0.2:=[profile?]
	dev-haskell/stm:=[profile?]
	>=dev-haskell/strict-0.3:=[profile?]
	dev-haskell/system-argv0:=[profile?]
	dev-haskell/system-filepath:=[profile?]
	dev-haskell/tar:=[profile?]
	>=dev-haskell/text-0.11:=[profile?]
	dev-haskell/transformers:=[profile?]
	dev-haskell/unordered-containers:=[profile?]
	dev-haskell/utf8-string:=[profile?]
	>=dev-haskell/uuid-1.3:=[profile?]
	dev-haskell/vector:=[profile?]
	>=dev-lang/ghc-7.6.1:=
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-1.16.0
	test? ( >=dev-haskell/setenv-0.1 <dev-haskell/setenv-0.2 )

	dev-haskell/happy
"

src_prepare() {
	# Creating tarfile is broken - it's already prebuilt in the tarball from
	# and building it again actually destroys it
	sed -i 's/create tarFile/-- create tarFile/' Setup.hs \
		|| die "unable to disable prebuild step"
}

