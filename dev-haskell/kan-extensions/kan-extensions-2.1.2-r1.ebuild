# Copyright 1999-2012 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: $

# ebuild generated by hackport 0.2.14

EAPI="4"

CABAL_FEATURES="lib profile haddock hoogle hscolour"
inherit haskell-cabal

DESCRIPTION="Kan extensions, the Yoneda lemma, and (co)density (co)monads"
HOMEPAGE="http://github.com/ekmett/kan-extensions/"
SRC_URI="http://hackage.haskell.org/packages/archive/${PN}/${PV}/${P}.tar.gz"

LICENSE="BSD"
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE=""

RDEPEND=">=dev-haskell/adjunctions-2.0.5[profile?]
		<dev-haskell/adjunctions-2.1[profile?]
		>=dev-haskell/comonad-1.1.1.1[profile?]
		<dev-haskell/comonad-1.2[profile?]
		>=dev-haskell/comonad-transformers-2.0.3[profile?]
		<dev-haskell/comonad-transformers-2.1[profile?]
		>=dev-haskell/comonads-fd-2.0.2.1[profile?]
		<dev-haskell/comonads-fd-2.1[profile?]
		>=dev-haskell/contravariant-0.1.2[profile?]
		<dev-haskell/contravariant-0.3[profile?]
		=dev-haskell/distributive-0.2*[profile?]
		>=dev-haskell/free-2.0.3[profile?]
		<dev-haskell/free-2.1[profile?]
		>=dev-haskell/keys-2.1.2[profile?]
		<dev-haskell/keys-2.2[profile?]
		>=dev-haskell/mtl-2.0.1.0[profile?]
		<dev-haskell/mtl-2.1[profile?]
		>=dev-haskell/representable-functors-2.1.2[profile?]
		<dev-haskell/representable-functors-2.2[profile?]
		>=dev-haskell/semigroupoids-1.2.6.1[profile?]
		<dev-haskell/semigroupoids-1.3[profile?]
		=dev-haskell/speculation-1.4*[profile?]
		=dev-haskell/transformers-0.2*[profile?]
		>=dev-lang/ghc-7.0.1"
DEPEND="${RDEPEND}
		>=dev-haskell/cabal-1.6"

src_prepare() {
	sed -e 's@contravariant          >= 0.1.2   && < 0.2@contravariant          >= 0.1.2   \&\& < 0.3@' \
		-i "${S}/${PN}.cabal" || die "Could not loosen dependencies"
}
