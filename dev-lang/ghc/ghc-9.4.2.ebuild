...
relocate_ghc() {
	...
	if [[ ${bin_libdir} != $(get_libdir) ]]; then
		...
		relocate_path "${D}/usr/${bin_libdir}" "/usr/$(get_libdir)" \
...
src_compile() {
	if ! use binary; then
		...
		hadrianopts="--flavour=quick --docs=none ${MAKEOPTS}"
		# wtf hadrian rebuilds half of ghc when installing
		#einfo "Running ./hadrian/build ${hadrianopts}"
		#./hadrian/build ${hadrianopts} || die
	fi # ! use binary
}
...
src_install() {
	if use binary; then
		...
		# WTF the cabal config gets reset if ran as `ebuild ghc-9.4.2.ebuild install`
		mkdir -p "${HOME}/.cabal"
		echo 'offline: True' > "${HOME}/.cabal/config"
