...
GHC_BRANCH_COMMIT="2b22b6ae69c94e721fde8af0108eb0feed97cc82" # ghc-9.6.7-release

GHC_BINARY_PV="9.4.3"
HADRIAN_BOOTSTRAP_TARBALL="${P}-hadrian-bootstrap-sources-${GHC_BINARY_PV}.tar.gz"
SRC_URI="
	https://downloads.haskell.org/~ghc/${PV}/${P}-src.tar.xz
	verify-sig? ( https://downloads.haskell.org/~ghc/${PV}/${P}-src.tar.xz.sig )
	!ghcbootstrap? (
		https://downloads.haskell.org/~ghc/9.6.7/hadrian-bootstrap-sources/hadrian-bootstrap-sources-${GHC_BINARY_PV}.tar.gz
...
BDEPEND="
	...
	ghcbootstrap? (
		...
		~dev-haskell/hadrian-${PV}[-vanilla(-)]
	)
	test? (
		${PYTHON_DEPS}
		${LLVM_DEPS}
...
src_prepare() {
	...
	pushd "${S}/hadrian" || die
		...
		eapply "${FILESDIR}/hadrian-9.6.7-add-packages.patch"
	popd
	...
	if use test; then
		eapply "${FILESDIR}/${PN}-9.6.3-fix-ipe-test.patch"
	...
	if ! use ghcbootstrap; then
		...
		( cd "${S}/hadrian/bootstrap" || die
			./bootstrap.py \
				...
				-s "${DISTDIR}/${HADRIAN_BOOTSTRAP_TARBALL}" || die "Hadrian bootstrap failed"
		)
	fi
