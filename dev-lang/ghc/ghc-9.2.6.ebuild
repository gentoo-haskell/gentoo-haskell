...
inherit pax-utils toolchain-funcs prefix
...
src_configure() {
	...
	if ! use binary; then
		...
		cp hadrian/src/UserSettings.hs hadrian/UserSettings.hs
		mkdir _build
		...
		if ! is_native; then
			hadrian_vars+="--docs=no-haddocks --docs=no-sphinx-pdfs --docs=no-sphinx-html "
		...
		# TODO: port to hadrian
		# if [[ -n ${GHC_LIBRARY_WAYS} ]]; then
		# 	echo "GhcLibWays=${GHC_LIBRARY_WAYS}" >> mk/build.mk
		# fi
		#echo "BUILD_PROF_LIBS = $(usex profile YES NO)" >> mk/build.mk
		use profile && sed -i -e 's/userDefaultFlavour = \"default\"/userDefaultFlavour = \"prof\"/' \
			hadrian/UserSettings.hs
		...
		cat hadrian/UserSettings.hs || die
