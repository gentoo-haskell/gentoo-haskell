...
src_prepare() {
	cabal_chdeps \
		'pandoc >= 1.9.0.5 && < 1.10' 'pandoc >= 1.9.0.5 && < 1.11' \
		'pandoc-types >= 1.9.0.2 && < 1.10' 'pandoc-types >= 1.9.0.2 && < 1.11'
	if has_version ">=app-text/pandoc-1.10"; then
		# This patch is from upstream, hopefully they will release a new version soon
		epatch "${FILESDIR}/${PN}-0.10.1.2-pandoc-1.10.patch"
	fi
}
