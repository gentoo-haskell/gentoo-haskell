...
RDEPEND="~dev-haskell/pandoc-3.7.0.2:=
	dev-haskell/text:=
	>=dev-lang/ghc-9.0.2:=
	lua? ( >=dev-haskell/hslua-cli-1.4.1:= <dev-haskell/hslua-cli-1.5:=
		>=dev-haskell/pandoc-lua-engine-0.4.3:= <dev-haskell/pandoc-lua-engine-0.5:=
		>=dev-haskell/temporary-1.1:= <dev-haskell/temporary-1.4:= )
	server? ( >=dev-haskell/pandoc-server-0.1.0.9:= <dev-haskell/pandoc-server-0.2:=
			dev-haskell/safe:=
